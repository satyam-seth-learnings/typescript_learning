...
class Game {
  ...
  //   display the numbers on which Math Game will be played
  displayNumbers(): void {
    document.getElementById("gameNumbers")!.innerHTML = `${String(
      this.randNum1
    )} and ${String(this.randNum2)}`;
  }
  ...
  displayGame(): void {
    ...
    for (let i = 0; i < this.operators.length; i++) {
      gameBoard += `<div class="form-group">
                      ...
                      ${String(this.randNum1)} ${this.operators[i]} ${String(
        this.randNum2
      )}
                      </label>
                      <div class="col-sm-1">
  ...
  calculateScore(): void {
    ...
    for (let i = 0; i < this.problemCount; i++) {
      let inputAnswer = Helper.getUserInput("answer" + (i + 1));

      // check if there is an input answer or it is left blank
      if (inputAnswer) {
        answer = Number(Helper.getUserInput("answer" + (i + 1)));
      } else {
        continue;
      }

      const correctAns = eval(
        this.randNum1 + this.operators[i] + this.randNum2
      );
